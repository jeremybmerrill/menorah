$(function(){
  ...
  $.ajax({
    url: 'http://www.hebcal.com/hebcal/?v=1&cfg=json&maj=on&min=on&mod=on&year='+now.year()+'&month=' + (now.month()+1) +'&c=on&callback=jsonp',
    dataType: 'jsonp',
    jsonpCallback: 'jsonp',
    success: function(data){
      hanukah_start_date = moment(_.find(data.items, function(i){ return i.title == "Chanukah: 1 Candle" }).date, 'YYYY-MM-DD')
      draw_menorah(hanukah_start_date);
    },
  });


function draw_menorah(hanukah_start_date){
  ...
  var shamash_height = 30;
    ...
    candle_count = Math.floor(now.diff(hanukah_start_date, "minutes") / (24 * 60)) + 1;
    next_candlelighting_time = hanukah_start_date.add("days", now.diff(hanukah_start_date, "days") );
    if( next_candlelighting_time.diff(now, "seconds") < 2){ //if the closest candlelighting time has already passed.
      next_candlelighting_time = hanukah_start_date.add("days", now.diff(hanukah_start_date, "days") + 1 )
    }
